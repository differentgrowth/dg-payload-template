...
export async function generateMetadata({
  ...
  const title = `${page.meta?.title || "Blog"} - Página ${pageNumber}`;
  const description =
    page.meta?.description ||
    "Descubre las últimas novedades y artículos sobre estrategias digitales, diseño web y SEO.";
  const canonicalUrl = `${serverUrl}/blog/page/${pageNumber}`;
...
export default async function Page({
  ...
  const [blogPage, { docs: posts, totalPages, hasNextPage, hasPrevPage }] =
    await Promise.all([getBlogPage(), getPosts({ page: sanitizedPageNumber })]);
